...
def extract_hotel_info(html):
    ...
    for idx, image_div in enumerate(soup.select('div.imagehtldis'), 1):
        ...
        if data_hotel_code:
            ...
            hotel_data['code'] = data_hotel_code

        if hotel_data not in hotel_info:
...
def fetchingInformationFromWeb(initial_num = 0, max_count = 100, search_id = 99790, rating = 0, city_name = "Mumbai"):
